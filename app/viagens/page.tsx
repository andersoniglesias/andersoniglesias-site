...
export const metadata = {
  title: "Viagens | Anderson Iglesias",
  description: "Dicas e relatos de viagens por Anderson Iglesias.",
};
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-14">
      ...
      <p className="mt-3 text-gray-600">
        Dicas, roteiros e aprendizados de viagens.
      </p>
          ...
          posts.map((p) => (
            <article key={p.id} className="rounded-2xl border bg-white p-6">
              ...
              <h2 className="mt-2 text-xl font-semibold">
                <Link className="underline" href={`/viagens/${p.slug}`}>
                  {p.title}
                </Link>
              </h2>
              {p.excerpt ? (
